...
class Sphere {
public:
  static IndexedTriangleList MakeTesselated(Vertex::Layout layout, int latDiv,
                                            int longDiv) {
    ...
    for (int iLat = 1; iLat < latDiv; iLat++) {
      ...
      for (int iLong = 0; iLong < longDiv; iLong++) {
        ...
        auto v = dx::XMVector3Transform(
            latBase, dx::XMMatrixRotationZ(longitudeAngle * iLong));
    ...
    const auto calcIdx = [longDiv](unsigned int iLat, unsigned int iLong) {
  ...
  static IndexedTriangleList
  Make(std::optional<Vertex::Layout> layout = std::nullopt) {
    if (!layout) {
